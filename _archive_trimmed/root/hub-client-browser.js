/**
 * Rivalis Live Hub Client - Browser/PWA Version
 * Works in any browser/PWA environment (no Node.js required)
 */

class RivalisHubClientBrowser extends EventTarget {
  constructor(config = {}) {
    super();
    ...
    // Custom event map for easier subscription
    this.listeners = {};
  }

  /**
   * Register event listener (for non-native events)
   */
    ...
    if (this.listeners[eventName]) {
      this.listeners[eventName].forEach(callback => {
        try {
          ...
          console.error(`[HubClient] Error in ${eventName} callback:`, error);
        }
      });
    }
    // Also dispatch native CustomEvent
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        this.ws.onerror = (event) => {
          console.error('[HubClient] WebSocket error:', event);
          this.connected = false;
  ...
  attemptReconnect() {
    ...
    console.log(`[HubClient] Reconnecting in ${delay}ms (attempt ${this.reconnectAttempts}/${this.maxReconnectAttempts})`);
    setTimeout(() => {
   ...
   * Called by Hub's MediaPipe after exercise is completed
   */
  submitRep(repData) {
    ...
    if (!repData.exercise || !repData.angles || !repData.distances) {
      console.error('[HubClient] Invalid rep data - missing required fields');
      return;
    }
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = RivalisHubClientBrowser;
}
